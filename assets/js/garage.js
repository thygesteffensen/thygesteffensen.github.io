document.addEventListener('DOMContentLoaded', () => {
  ...
  document.head.innerHTML = `
    ...
    <style>
      ...
      .status-sub  { font-size: 13px; color: #aaa; }
      button {
  ...
  document.body.innerHTML = `
    <div class="card">
      ...
      <span id="status-sub" class="status-sub">forbinder…</span>
      <button id="trigger-btn" onclick="triggerDoor()" disabled>
        <div class="spinner"></div>
        <span class="btn-label">Åbn / luk / stop</span>
      </button>
    </div>
  `;
  ...
  function applyState(isOpen) {
    ...
    textEl.textContent = isOpen ? 'Garage åben' : 'Garage lukket';
    subEl.textContent  = isOpen ? 'Sensor: åben' : 'Sensor: lukket';
  }
